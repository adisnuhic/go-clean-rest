...
func TestGetByEmail_Success(t *testing.T) {
	...
	// Set up expectations on the mock
	mockRepo.On("GetByEmail", "test@test.com").Return(expectedUser, nil)

	// Create a userService instance with the mocked repository
	userService := NewUserService(nil, mockRepo)

	// Call the GetByEmail method on the userService
	user, err := userService.GetByEmail("test@test.com")

	// Assert that the user and error returned by the service match the expectations
	assert.Empty(t, err)
	assert.Equal(t, expectedUser, user)

	// Ensure that the GetByEmail method of the mock was called with the correct arguments
	mockRepo.AssertCalled(t, "GetByEmail", "test@test.com")
}
...
func TestGetByEmail_Error(t *testing.T) {
	...
	// Set up expectations on the mock
	mockRepo.On("GetByEmail", "test2@test.com").Return(nil, expectedError)

	// Create a userService instance with the mocked repository
	userService := NewUserService(nil, mockRepo)

	// Call the GetByEmail method on the userService
	user, errUser := userService.GetByEmail("test2@test.com")

	// Assert that the user and error returned by the service match the expectations
	assert.Empty(t, user)
	assert.Equal(t, expectedError, errUser)

	// Ensure that the GetByEmail method of the mock was called with the correct arguments
	mockRepo.AssertCalled(t, "GetByEmail", "test2@test.com")
}
